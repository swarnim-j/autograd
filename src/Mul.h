...
template<typename T>
std::vector<std::shared_ptr<Tensor<T>>> Mul<T>::forward(const std::vector<std::shared_ptr<Tensor<T>>>& inputs) {
    if (inputs.size() != 2) {
...
template<typename T>
std::vector<std::shared_ptr<Tensor<T>>> Mul<T>::backward(const std::vector<std::shared_ptr<Tensor<T>>>& grad_outputs) {
    ...
    if (a->requires_grad) {
        ...
        for (size_t i = 0; i < grad_output->data.size(); ++i) {
            grads[0]->data[i] = grad_output->data[i] * b->data[i];
        }
    }
    ...
    if (b->requires_grad) {
        ...
        for (size_t i = 0; i < grad_output->data.size(); ++i) {
            grads[1]->data[i] = grad_output->data[i] * a->data[i];
        }
    }
